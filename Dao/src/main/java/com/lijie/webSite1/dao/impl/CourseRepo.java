...
public class CourseRepo implements ICourseRepo{
    ...
    public List<CourseVO> getCourseVOsByStudentId(String stuId) throws WebException{
        String sqlString="select student_course.stuId as stuId,student_course.grade as grade, course.id as id,course.name as name,course.teacherId as teacherId,course.description as description, course.startDate as startDate,course.endDate as endDate,teacher.name as teacherName, student.name as stuName from student_course,course,teacher,student where student_course.stuId=? and student_course.courseId=course.id and course.teacherId=teacher.id and student.id=student_course.stuId";
    ...
    public List<CourseVO> getCourseVOsByCourseId(String courseId) throws WebException {
        String sqlString="select student_course.stuId as stuId,student_course.grade as grade, course.id as id,course.name as name,course.teacherId as teacherId,course.description as description, course.startDate as startDate,course.endDate as endDate,teacher.name as teacherName, student.name as stuName from student_course,course,teacher,student where student_course.courseId=? and student_course.courseId=course.id and course.teacherId=teacher.id and student.id=student_course.stuId";
        List<CourseVO> courseVOs =null;
        try{
            courseVOs = jdbcTemplate.query(sqlString,new Object[]{courseId},new CouseDtoRowMapper());
        }catch (Exception e)
        {
            e.printStackTrace();
            throw new WebException(2,"数据库访问出错");
